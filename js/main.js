...
function getAllRecords() {
    fetch('//34.229.231.213/php-intro-connection/getRecords.php') // Cambia localhost a tu IP pública
        .then(response => {
            if (!response.ok) {
...
function getAllRecordsByCountry() {
    fetch('//34.229.231.213/php-intro-connection/getRecordsByCountry.php') 
        .then(response => {
            if (!response.ok) {
                throw new Error('Error en la respuesta de la red: ' + response.status);
            }
...
function getAllRecordsByContinent() {
    fetch('//34.229.231.213/php-intro-connection/getRecordsByContinent.php') 
        .then(response => {
            if (!response.ok) {
                throw new Error('Error en la respuesta de la red: ' + response.status);
            }

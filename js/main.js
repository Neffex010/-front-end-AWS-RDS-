...
function getAllRecords() {
    fetch('//52.55.4.190/php-intro-connection/getRecords.php') // Cambia localhost a tu IP pública
        .then(response => {
            if (!response.ok) {
...
function getAllRecordsByCountry() {
    fetch('//52.55.4.190/php-intro-connection/getRecordsByCountry.php') 
        .then(response => {
            if (!response.ok) {
                throw new Error('Error en la respuesta de la red: ' + response.status);
            }
...
function getAllRecordsByContinent() {
    fetch('//52.55.4.190/php-intro-connection/getRecordsByContinent.php') 
        .then(response => {
            if (!response.ok) {
                throw new Error('Error en la respuesta de la red: ' + response.status);
            }
